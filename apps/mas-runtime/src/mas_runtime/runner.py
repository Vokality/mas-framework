...
from mas_gateway.config import GatewaySettings, validate_gateway_config
from mas_server import AgentDefinition, MASServer, MASServerSettings, TlsConfig
from mas_core.telemetry import TelemetryConfig, configure_telemetry, get_telemetry
...
class AgentRunner:
    ...
    def _load_agent_class(class_path: str) -> type[Agent[Any]]:
        ...
        if not issubclass(target, Agent):
            raise TypeError(f"{class_path} is not a mas_agent.Agent subclass")
