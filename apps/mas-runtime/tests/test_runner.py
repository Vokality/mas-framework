...
from mas_runtime._test_support import NoopAgent
from mas_runtime.runner import AgentRunner, AgentSpec, RunnerSettings
...
def _write_mas_yaml(path: Path, *, gateway_url: str | None = None) -> None:
    ...
    lines.extend(
        [
            "agents:",
            "  - agent_id: test_agent",
            "    class_path: mas_runtime._test_support:NoopAgent",
            "    instances: 1",
...
def test_load_agent_class_validation() -> None:
    with pytest.raises(ValueError, match="module:ClassName"):
        AgentRunner._load_agent_class("mas_runtime._test_support.NoopAgent")

    with pytest.raises(TypeError, match="mas_agent.Agent subclass"):
        AgentRunner._load_agent_class("mas_runtime._test_support:NotAnAgent")

    loaded = AgentRunner._load_agent_class("mas_runtime._test_support:NoopAgent")
...
async def test_runner_normalizes_wildcard_server_addr_for_agents() -> None:
    settings = RunnerSettings(
        ...
        agents=[
            AgentSpec(
                agent_id="addr",
                class_path="mas_runtime._test_support:AddressCaptureAgent",
                instances=1,
                tls_cert_path="tests/certs/sender.pem",
                tls_key_path="tests/certs/sender.key",
            )
